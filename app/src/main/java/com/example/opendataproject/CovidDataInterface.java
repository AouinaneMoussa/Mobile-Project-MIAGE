...
public interface CovidDataInterface {
    ...
    Call<CovidResponse> getCovidData(
            @Query("rows") int rows, // Paramètre pour limiter le nombre de résultats
            @Query("start") int start,// Paramètre pour la pagination
            @Query("where") String where // Nouveau paramètre pour le filtre
    );
}